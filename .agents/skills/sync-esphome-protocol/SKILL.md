...
5. Inspect the protocol inventory diff before handwritten code. Classify additions, removals, field changes, enum changes, and message-ID changes.
6. Update `protocol/upstream.lock.json`, generated wire output, inventory, `THIRD_PARTY_NOTICES.md` when needed, and `docs/support-matrix.md` in one PR.
...
8. Run clean regeneration, policy validation, unit/integration tests, race tests, fuzz smoke, and compatibility tests required by the changed surface.
