...
- Endless no-hardware blink loop, indistinguishable from hardware to MGMT: [MGMT blink loop demo](docs/mgmt-blink-demo.md)
...
- Message-by-message gates and evidence: [protocol compatibility inventory](docs/protocol-inventory.md)
...
The conveyor demonstration is the first visible acceptance system, not the library architecture. The device reports two facts and accepts two commands; MGMT owns every decision. See the [architecture](docs/architecture.md), the [conveyor profile](docs/conveyor-demo.md), and the [MGMT simulator demo](docs/mgmt-simulator-demo.md) to run it.

## Repository status

The repository is public and GPL-3.0-only licensed. The original immutable MGMT baseline is recorded in [`compatibility/mgmt-feat-esphome.json`](compatibility/mgmt-feat-esphome.json), the replacement candidate is recorded in [`compatibility/mgmt-feat-esphome2.json`](compatibility/mgmt-feat-esphome2.json), and append-only runtime proofs cover the [conveyor](compatibility/mgmt-feat-esphome2-runtime.json), [both original baseline examples plus polling/reconnect](compatibility/mgmt-feat-esphome2-baselines.json), the [post-merge `feat/esphome` branch](compatibility/mgmt-feat-esphome-postmerge.json), [real `.local` mDNS parity](compatibility/mgmt-feat-esphome-mdns.json), the [final MGMT diagnostics pin](compatibility/mgmt-feat-esphome-diagnostics.json), the [first sanitized real-hardware blink run](compatibility/mgmt-feat-esphome-hardware-blink.json), the [green upstream MGMT PR ready for review](compatibility/mgmt-upstream-pr-961-ready.json), and the [single-owner conveyor contract](compatibility/mgmt-feat-esphome-conveyor-contract.json). The ESPHome 2026.7.0 wire surface is pinned in [`protocol/upstream.lock.json`](protocol/upstream.lock.json) and classified message by message in the [generated compatibility inventory](protocol/inventory.json). Run the no-hardware quickstart in the [cheatsheet](CHEATSHEET.md).
