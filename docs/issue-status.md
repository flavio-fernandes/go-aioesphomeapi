...
| [#10](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/10) simulator fault engine | The manual clock/latest-state timeline, ADR 0013 pinned MGMT re-baseline, ordered command expectations, and slow-subscriber saturation are complete. Add conditional seed validation when randomized actions arrive, delayed/fragmented/coalesced shaping, and broader owned-resource cleanup assertions. |
