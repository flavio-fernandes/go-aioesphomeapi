...
| [#7](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/7) public-repository controls | `main` now requires PRs, strict `go`/`validate`, stale-review dismissal, conversation resolution for administrators, and forbids force-push/deletion. Verify secret scanning/push protection, private reporting, dependency updates, emergency bypass, and the future independent-approval/CODEOWNERS gate before closing. |
