...
| [#5](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/5) protocol inventory views | close | The reviewed annotation schema and generated inventory classify all 148 unique IDs by version/feature gate, entity family, MGMT need, reference parity, public behavior, and every evidence level. All 33 M1 messages are accounted for with typed/simulated proof; DeviceInfo request/response gained theirs when #11 landed the device information exchange. Unknown IDs/enums/fields have separate status and test plans. |
...
| [#11](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/11) context-bound connection lifecycle | close | Hello/API version, typed `DeviceInfo`, caller `Ping`, opt-in bounded `WithKeepalive`, peer disconnect, `Done`/`CloseReason`, and idempotent close are typed and race-tested against the simulator. A manual-clock test orders the established, subscribed, and closed phases with no wall-clock ordering; counting dialers prove one dial per owner with no automatic redial; owned-goroutine budgets reach zero after deliberate close, keepalive loss, and peer loss; probe and device-info completions bypass the subscriber queue so a slow callback cannot fake a dead peer; an interrupted command fails typed with no replay on the next owner's connection. Keepalive stays disabled in the MGMT facade path, whose session owns liveness and reconnect, and the pinned MGMT lane proves the wrapper's reconnect, polling, and outage tracking unchanged. |
| [#12](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/12) release-candidate verification | close | PR #69 landed all five lanes: the three-target pull-request fuzz smoke plus the finite scheduled/dispatch extended fuzz job, generated-drift CI, documented owned-goroutine budgets (three per connected client, one per accepted simulator connection, zero after close) with allocation ceilings, the fail-closed dependency/module/license report, and the pinned-MGMT lane using MGMT's `noaugeas novirt` tags. Hosted evidence is complete: pull-request runs of policy, Go, generated-drift, and the MGMT lane pass; the first dispatched hosted extended fuzz execution from `main` (run 29665041850, 2026-07-18) passes all three 10-minute targets; the MGMT lane passes on every `main` push run since the merge (first: run 29665039188, 2026-07-18) and stays on the weekly schedule. The merge-audit gap found during closure (Codex clean comment verdicts unrecognized) is fixed in the review skill. |
...
| [#7](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/7) public-repository controls | Checked-in policy keeps paid Codex review optional while requiring PRs, strict `go`/`validate`, thread-aware conversation resolution, and expected-head merges. Remove `codex-review` from the `main` ruleset to match, then verify stale-review dismissal, force-push/deletion denial, secret scanning/push protection, private reporting, dependency updates, emergency bypass, and the future independent-approval/CODEOWNERS gate before closing. |
...
| [#14](https://github.com/flavio-fernandes/go-aioesphomeapi/issues/14) interactive conveyor demo | Pushed sensor changes now drive the whole story: the simulated firmware runs bricks down the belt, MGMT owns the motor and the light, and both classifications are asserted. Still remaining: network interruption and safe stop, contradictory-sensor and slow-subscriber faults, responsibility display, presenter runbook, and sanitized physical recovery checklist. |
...
2. Surface #40 so MGMT can attribute asynchronous connection loss without
   coupling its session layer to the concrete library client.
3. Close the remaining security budget gate in #6.
