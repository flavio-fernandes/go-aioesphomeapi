...
The MIT library must not import MGMT. MGMT may import this library. Integration code that implements MGMT interfaces belongs under MGMT's GPL-compatible repository and follows MGMT's `Default`, `Validate`, `Init`, `Watch`, `CheckApply`, and `Cleanup` lifecycle.
