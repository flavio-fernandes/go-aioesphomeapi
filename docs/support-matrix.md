...
The evidence is append-only: [`compatibility/mgmt-feat-esphome2.json`](../compatibility/mgmt-feat-esphome2.json) records the first candidate, while [`compatibility/mgmt-feat-esphome-review.json`](../compatibility/mgmt-feat-esphome-review.json) records the final reviewed branches. The [timeline candidate record](../compatibility/mgmt-feat-esphome-simulator-timeline-candidate.json) preserves ADR 0013's measured before/after reconnect behavior, and the [post-merge record](../compatibility/mgmt-feat-esphome-simulator-timeline-postmerge.json) pins the exact merged revisions. Historical rows remain so a successful build is never mistaken for later runtime or hardware evidence.
...
| Release verification lanes (PR #69) | hosted pull-request runs of repository policy, the Go job (race, three-target fuzz smoke, dependency report), and generated-drift pass; owned-goroutine and allocation budget tests pass under the race detector | Library CI evidence only; no MGMT or hardware claim. |
| Extended fuzz, 10 minutes per target | pass locally on 2026-07-18 for `FuzzPlainFramerRead`, `FuzzDecode`, and `FuzzAnswerIP`, and the first hosted dispatched execution from `main` (Repository policy run 29665041850, 2026-07-18) passes all three targets | Hosted dispatch evidence recorded; the weekly schedule continues. |
| Pinned MGMT hosted lane | pass end-to-end locally and hosted with MGMT's `noaugeas novirt` tags: pin and tree-hash verification, patched build, targeted race lanes, and all reviewed MCL scenario convergences pass as a pull-request check and on every `main` push run since the merge (first: MGMT compatibility run 29665039188, 2026-07-18) | Hosted evidence recorded; the weekly schedule continues. |
...
| Device information | known | typed | simulated | none | none | M1 |
| Ping, keepalive, disconnect, close | known | typed | simulated | none | none | M1 |
...
| Defensive scenario creation | typed | simulated | n/a | M1 | Valid protobuf entities, initial states, timeline events, logs, commands, protocol faults, and network faults are cloned before the device can observe caller mutation. An exact reflected field inventory fails when `Scenario` grows until validation and cloning receive explicit review. |
...
| Device info and probe accounting | typed | simulated | n/a | M1 | The device answers `DeviceInfoRequest` with fixed synthetic identity values (a locally administered MAC constant and the pinned protocol version) and counts answered pings in `DeviceStats.AnsweredPings`, so device-info and keepalive tests assert exact observable behavior without any real device identifier. |
