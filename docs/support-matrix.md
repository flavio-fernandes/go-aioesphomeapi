...
| Extended fuzz, 10 minutes per target | pass locally on 2026-07-18 for `FuzzPlainFramerRead`, `FuzzDecode`, and `FuzzAnswerIP`; the three fuzzed code paths are byte-identical after the rebase onto current `main` | Local evidence only until the first hosted scheduled or dispatched run. |
| Pinned MGMT hosted lane | pass end-to-end locally with MGMT's `noaugeas novirt` tags: pin and tree-hash verification, patched build, targeted race lanes, and all reviewed MCL scenario convergences | Local evidence; the first hosted execution is a PR #69 check and a post-merge dispatch. |
